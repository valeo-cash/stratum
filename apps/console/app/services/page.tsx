import { getServices } from "../lib/gateway";
...
export default async function ServicesPage() {
  ...
  return (
    <div className="p-6 lg:p-10 max-w-[1200px]">
      ...
      {services.length === 0 && (
        <p className="text-[#9CA3AF] text-sm py-12 text-center">No services registered yet. Use the Gateway admin API to register services.</p>
      )}

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {services.map((svc) => (
          <Link key={svc.slug} href={`/services/${svc.slug}`} className="block group">
