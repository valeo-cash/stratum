...
export default async function WindowsPage() {
  ...
  const allWindows = [
    ...(stats.currentWindow ? [{ ...stats.currentWindow, merkleRoot: undefined, anchorTxHash: undefined, finalizedAt: undefined }] : []),
  ...
  return (
    <div className="p-6 lg:p-10 max-w-[1200px]">
          ...
          <tbody>
            {allWindows.map((w) => (
              <tr key={w.windowId} className="border-t border-[#E5E7EB] hover:bg-[#FAFAFA] transition-colors">
                <td className="py-3">
                  <Link href={`/windows/${w.windowId}`} className="text-[#003FFF] font-mono text-xs hover:underline">{w.windowId}</Link>
                </td>
                <td className="py-3">
      ...
      {allWindows.length === 0 && (
        <p className="text-[#9CA3AF] text-sm py-12 text-center">No windows yet. Start the simulator to trigger settlement.</p>
      )}
    </div>
  );
}
