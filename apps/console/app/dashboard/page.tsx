...
const ReceiptsChart = dynamic(() => import("../components/ReceiptsChart"), {
  ssr: false,
});
...
export default async function DashboardPage() {
  ...
  // Build chart data: receipts per hour for last 24h
  const now = Date.now();
  const chartData = [];
  for (let i = 23; i >= 0; i--) {
  ...
  const stats = [
    ...
    },
    {
      ...
      value: `${avgCompression.toFixed(1)}:1`,
      icon: TrendingUp,
      color: "#6366F1",
    },
  ];
  ...
  return (
    <div className="p-6 lg:p-10 max-w-[1200px]">
      ...
      <p className="text-[#6B7280] text-sm mb-8">
        Overview of your Stratum clearing activity.
      </p>
        ...
        {stats.map((stat) => {
          ...
          return (
            ...
            >
              ...
              <p
                className="text-[#0A0A0A] font-mono"
      ...
      {currentWindow && (
        <div className="bg-[#FAFAFA] border border-[#E5E7EB] rounded-none p-5 mb-8">
          <div className="flex items-center justify-between mb-4">
            ...
            <span
              className={`text-[11px] font-mono uppercase tracking-[0.1em] px-2 py-0.5 rounded-none ${
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-6">
            <div>
              <p className="text-[11px] text-[#9CA3AF] mb-1">Window ID</p>
            ...
            <div>
              ...
              <p className="text-sm font-mono text-[#0A0A0A]">
                {currentWindow.receiptCount}
              </p>
            </div>
            <div>
              <p className="text-[11px] text-[#9CA3AF] mb-1">Gross Volume</p>
              <p className="text-sm font-mono text-[#0A0A0A]">
                ${currentWindow.grossVolume.toFixed(2)}
              </p>
            </div>
            <div>
              <p className="text-[11px] text-[#9CA3AF] mb-1">Opened</p>
              <p className="text-sm font-mono text-[#0A0A0A]">
                {currentWindow.openedAt.toLocaleDateString()}
              </p>
            </div>
          </div>
        </div>
