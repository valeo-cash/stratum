import { getReceipt, getReceipts } from "@/app/lib/gateway";
import { redirect } from "next/navigation";
import Link from "next/link";
import SearchBar from "@/app/components/explorer/SearchBar";
            ...
            <tbody>
              {matches.slice(0, 50).map((r: any) => (
                <tr key={r.id} className="border-t border-[#E5E7EB]">
                  <td className="py-2.5">
                    <Link href={`/explorer/receipt/${r.id}`} className="text-[#003FFF] font-mono text-xs hover:underline">
                      {r.id}
                    </Link>
                  </td>
                  ...
                  <td className="py-2.5">
                    <Link href={`/explorer/window/${r.windowId}`} className="text-[#003FFF] font-mono text-xs hover:underline">
                      {r.windowId}
                    </Link>
                  </td>
